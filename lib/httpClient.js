...
HttpClient.prototype.getQS = function (allowedProps, args) {
  ...
  for (var i = 0; i < allowedProps.length; i++) {
    if (Object.prototype.hasOwnProperty.call(safeArgs, allowedProps[i])) {
      const value = safeArgs[allowedProps[i]]
      // Strava honors only the last occurrence of a repeated query param, so list
      // values must go out as one comma-separated value, not key=a&key=b.
      qa[allowedProps[i]] = Array.isArray(value) ? value.join(',') : value
    }
  }
