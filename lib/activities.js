...
var _createAllowedProps = [
  ...
  'description',
  'distance',
  // Current create docs omit this; keep sending it so existing JS callers do not regress.
  'private',
  'trainer',
  'commute'
]
 ...
 * otherwise cursor pagination (getCursorPaginationQS).
...
activities.prototype._listHelper = function (endpoint, args) {
  var isKudoers = endpoint.indexOf('/kudos') !== -1
  var useOffsetPagination =
    isKudoers ||
    ('page' in args && args.page !== undefined) ||
    ('per_page' in args && args.per_page !== undefined)
  var qs
  if (useOffsetPagination) {
    ...
    qs = this.client.getCursorPaginationQS(
